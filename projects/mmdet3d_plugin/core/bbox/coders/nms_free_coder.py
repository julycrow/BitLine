...
class BitLineNMSFreeCoder(BaseBBoxCoder):
    ...
    def __init__(self,
                 ...
                 *args, 
                 **kwargs):
        super(BitLineNMSFreeCoder, self).__init__(*args, **kwargs)
    ...
    def decode_single(self, cls_scores, bbox_preds, pts_preds, sm_preds):
        ...
        if self.post_center_range is not None:
            ...
            query_index = index.cpu()  # 在被循环变量覆盖前保存，用于GIF可视化
            ...
            # =========================================================
            # 关键修复：分离 mAP 用坐标 与 拓扑用坐标
            # ---------------------------------------------------------
            # 端点合并（endpoint merging）修改 pts 的首尾坐标，使相邻
            # 线段共享端点，从而构建连通图（APLS/SDA 依赖此步骤）。
            # 但被修改的 pts 同时用于 mAP 计算，破坏了几何精度。
            #
            # 解决方案：
            #   pts_for_mAP  ← 合并前克隆，保留原始几何坐标（用于 mAP）
            #   pts          ← 就地合并端点（用于图结构/拓扑指标）
            # =========================================================
            pts_for_mAP = pts.clone()  # 快照：保留原始坐标用于 mAP
            ...
            predictions_dict = {
                'line_scores': scores,
                'line_labels': labels,
                'line': pts_for_mAP.cpu(),  # 使用原始未合并坐标，保证 mAP 不受拓扑合并影响
                ...
                'graph': G, 
                'adj_matrix': sm,
                'query_index': query_index,  # GIF可视化用：保存过滤后的query索引（对应50×50邻接矩阵）
            }
