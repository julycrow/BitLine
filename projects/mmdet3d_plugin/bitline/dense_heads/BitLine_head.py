...
class CGTopoHead(DETRHead):
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        for lvl in range(hs.shape[0]):
            ...
            reference = inverse_sigmoid(reference)  # torch.Size([1, 1000, 2])

            outputs_class = self.cls_branches[lvl](hs[lvl]
                                            .view(bs,self.num_vec, self.num_pts_per_vec,-1)
                                            .mean(2))
            tmp = self.reg_branches[lvl](hs[lvl])  # torch.Size([1, 4, 1000, 256])-> torch.Size([4, 1000, 2])
            ...
            out_sm = self.lclc_branch(vertex_feat_out)  # [1, 50, 50]
    ...
    def loss(self,
        ...
        # import pdb;pdb.set_trace()
        all_cls_scores = preds_dicts['all_cls_scores']  # torch.Size([6, 4, 50, 1])
        all_bbox_preds = preds_dicts['all_bbox_preds']
        all_pts_preds  = preds_dicts['all_pts_preds']  # torch.Size([6, 4, 50, 20, 2])
        all_sms_preds  = preds_dicts['all_sms_preds']  # torch.Size([6, 4, 50, 50]) 预测50个实例之间的拓扑关系
        all_hs_preds = preds_dicts['all_hs']  # torch.Size([6, 4, 1000, 256])
        kp_bev_preds = preds_dicts['kp_bev_preds']  # torch.Size([4, 1, 200, 100])
        ...
        gt_sms_list = [torch.from_numpy(gt_bboxes.adj_matrix).to(device) for gt_bboxes in gt_vecs_list]  # 新增拓扑 GT
        gt_control_pts_list = [gt_bboxes.get_beizer_control_pts.to(device) for gt_bboxes in gt_vecs_list]  # 新增控制点 GT
        ...
        all_gt_sms_list = [gt_sms_list for _ in range(num_dec_layers)]  # 6,4,N,N  N为本场景中的车道数量
        all_gt_control_pts_list = [gt_control_pts_list for _ in range(num_dec_layers)]  # 6,4,N,N,4,2  10不固定,可能是11,14等
        ...
        gt_bev_kp_list = [self.get_bev_keypoint(gt_bboxes).to(device) for gt_bboxes in gt_vecs_list]  # 用椭圆形核（大小由 dilate_radius 控制）在 bev_keypoint_map 上做膨胀操作，把离散的点扩成小圆斑
    ...
    def _get_target_single(self,
        ...
        # adj matrix targets
        sm_pred = sm_pred[pos_inds][:, pos_inds]  # 提取正样本间的关系子图,预测的邻接矩阵：只保留正样本之间的连接
        sm_target = gt_sms[pos_gt_inds][:, pos_gt_inds].to(torch.float32)  # GT 邻接矩阵：根据匹配索引提取对应的 GT 子图

        # connectivity loss
        loss_adj = self.loss_adj(sm_pred, sm_target) * self.weight_adj  # 计算二元交叉熵损失

        # beizer loss
        pred_control_pts = torch.zeros((len(pos_inds), len(pos_inds), self.nums_ctp, 2)).to(pts_pred.device)  # Step 1: 初始化预测控制点张量
        gt_control_pts = gt_control_pts[pos_gt_inds][:, pos_gt_inds]  # Step 2: 提取 GT 控制点子图
        hs_pred_permute = hs_pred.view(self.num_vec, self.num_pts_per_vec,-1)[pos_inds]  # Step 3: 提取正样本的实例级特征 (来自 decoder 输出)
        for i in range(len(sm_target)):  # Step 4: 遍历 GT 邻接矩阵中的连接关系
            connection = torch.where(sm_target[i]==1)[0]
            for c in connection:  #  对每条连接的车道对 (i, c)
                new_line_embed = torch.cat((hs_pred_permute[i], hs_pred_permute[c]))  # 4.1 拼接两条车道的特征 (模拟连续车道线)
                beizer_space_embed = torch.matmul(self.inv_B.to(pts_pred.device), new_line_embed)  # 4.2 通过 Bernstein 基逆矩阵映射到控制点空间
                control_pts = self.beizer_transform(beizer_space_embed)  # 4.3 MLP 解码为控制点坐标
                pred_control_pts[i, c] = denormalize_2d_pts(torch.sigmoid(control_pts), self.pc_range)  # 4.4 Sigmoid + 反归一化到物理空间
        loss_beizer = self.loss_ctp(pred_control_pts, 
                                    gt_control_pts,
                                    avg_factor=len(pos_inds))  # Step 5: 计算控制点回归损失 (L1 Loss)
    ...
    def get_bboxes(self, preds_dicts):
        ...
        for i in range(num_samples):
            ...
            query_index = preds.get('query_index', None)  # GIF可视化用
            ret_list.append([line_scores, line_labels, line,
                            pts_scores, pts_labels, pts, graphs, adj_matrix, query_index])
...
def _neg_loss(pred, gt, weights=None):
    ...
    """
    pos_inds = gt.eq(1).float()  # 找出等于 1 的位置，正样本
    neg_inds = gt.lt(1).float()  # 找出小于 1 的位置，负样本
