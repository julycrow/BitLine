...
class GeometrySptialCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        for j in range(bs):
            for i, reference_points_per_img in enumerate(reference_points_cam):  # train中是torch.Size([6, 4, 10000, 4, 2]) test中是torch.Size([6, 1, 40000, 1, 2])
                index_query_per_img = indexes[i]
                queries_rebatch[j, i, :len(
                    index_query_per_img)] = query[j, index_query_per_img]  # torch.Size([4, 6, 3914, 256]) (-3.7014,5.0050)
                reference_points_rebatch[j, i, :len(
                    index_query_per_img)] = reference_points_per_img[j, index_query_per_img]  # torch.Size([4, 6, 3914, 4, 2]) (-3.7259,3.5880)
...
class GeometryKernelAttention(BaseModule):
    ...
    def forward(self,
        ...
        bs, num_value, _ = value.shape  # torch.Size([24, 375, 256])
