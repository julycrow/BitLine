    ...
    def forward(self,
        ...
        if tgt is not None:
            ...
            for layer in self.operation_order:
                ...
                if layer == 'self_attn':
                    # 使用原始BEV空间的坐标和spatial_shapes
                    # TemporalSelfAttention内部会自动检测稀疏模式并调整
                    tgt = self.attentions[attn_index](
                        ...
                        reference_points=ref_2d,  # 使用原始坐标
                        spatial_shapes=torch.tensor([[bev_h, bev_w]], device=tgt.device),  # 使用原始spatial_shapes
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            if use_sparse:
                ...
                # 保持原始BEV空间的坐标，不重新归一化
                # TemporalSelfAttention内部会自动处理稀疏模式
