...
def pts_l1_loss(pred, target):
    ...
    if pred.size() != target.size():
        print(f"[DEBUG] pts_l1_loss size mismatch:")
        print(f"  pred.size() = {pred.size()}")
        print(f"  target.size() = {target.size()}")
        # 尝试reshape使其匹配
        if pred.numel() == target.numel():
            print(f"  Elements match, reshaping target to pred's shape")
            target = target.reshape(pred.size())
        else:
            raise AssertionError(f"Size mismatch: pred {pred.size()} vs target {target.size()}")
