...
def create_graph_midpoints(G_, linestring_delta=50, is_curved_eps=0.03,
    ...
    for u, v, data in G_.edges(data=True):
        ...
        if 'geometry' in data:
            ...
            # ignore if almost straight or if linelen is zero to avoid division by zero
            if linelen > 0 and np.abs(dst - linelen) / linelen < is_curved_eps:
...
def add_travel_time(G_, speed_key='inferred_speed_mps', length_key='length',
    ...
    for i, (u, v, data) in enumerate(G_.edges(data=True)):
        if speed_key in data:
            ...
            if type(speed) == list:
                # Use default_speed if list is empty to avoid "mean of empty slice" warning
                speed = np.mean(speed) if len(speed) > 0 else default_speed
            # print("speed:", speed)
        else:
