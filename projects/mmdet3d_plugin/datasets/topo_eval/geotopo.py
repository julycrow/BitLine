...
# Filter specific numpy warnings that don't affect results
...
class Evaluator():
    ...
    def topoMetric(self, thr=8, mask=None, verbose=False):
        ...
        if verbose:
            ...
            print("topo precision : ", float(matched) / len(prop_nodes) * (np.mean(ps) if len(ps) > 0 else 0.0))
            print("topo recall : ", float(matched) / len(gt_nodes) * (np.mean(rs) if len(rs) > 0 else 0.0))
        ...
        # Use 0.0 as default when lists are empty to avoid "mean of empty slice" warning
        topo_precision = float(matched) / len(prop_nodes) * (np.mean(ps) if len(ps) > 0 else 0.0)
        topo_recall = float(matched) / len(gt_nodes) * (np.mean(rs) if len(rs) > 0 else 0.0)
        topo_f1 = f1_score(topo_precision, topo_recall)

        jtopo_precision = float(matched) / len(prop_nodes) * (np.mean(jps) if len(jps) > 0 else 0.0)
        jtopo_recall = float(matched) / len(gt_nodes) * (np.mean(jrs) if len(jrs) > 0 else 0.0)
