...
class LiDARInstanceLines(object):
    ...
    def fixed_num_sampled_points_ambiguity(self):
        ...
        for instance in self.instance_list:
            ...
            instance_points_list.append(sampled_points)
        instance_points_array = np.array(instance_points_list)
        is_3d = instance_points_array.shape[-1] == 3
        instance_points_tensor = to_tensor(instance_points_array)
        instance_points_tensor = instance_points_tensor.to(
                            dtype=torch.float32)
    ...
    def get_beizer_control_pts(self):
        ...
        if self.adj_matrix is None:
            return torch.zeros((0, 0, self.nums_control_pts, 2), dtype=torch.float32)

        instance_points = self.fixed_num_sampled_points[:, :, :2]
        B = self.get_bernstein_basis(self.fixed_num * 2, self.nums_control_pts)
        nums_line = len(self.adj_matrix)
        control_pts_matrix = torch.zeros((nums_line, nums_line, self.nums_control_pts, 2), dtype=torch.float32)
        inv_B = torch.linalg.pinv(B)
        for i in range(nums_line):
            connection = np.where(self.adj_matrix[i] == 1)[0]
            for idx in connection:
                merged = torch.cat((instance_points[i], instance_points[idx]), dim=0)
                control_pts_matrix[i, idx] = torch.matmul(inv_B, merged)
    ...
    def get_bernstein_basis(self, nums_pts, nums_ctp):
        ...
        B = torch.zeros((nums_pts, nums_ctp), dtype=torch.float32)
        t = torch.arange(nums_pts, dtype=torch.float32) / (nums_pts - 1)
        for i in range(nums_pts):
            for j in range(nums_ctp):
                B[i, j] = comb(nums_ctp - 1, j) * (1 - t[i]) ** (nums_ctp - 1 - j) * t[i] ** j
        return B
...
class VectorizedAV2LocalMap(object):
    ...
    def __init__(self,
        ...
        self.padding_value = padding_value
        self.only_centerline = only_centerline
        self.nums_control_pts = nums_control_pts
    ...
    def gen_vectorized_samples(self, location, map_elements, lidar2global_translation, lidar2global_rotation):
        ...
        for vec_class in self.vec_classes:
            if vec_class == 'divider':
                divider_data = map_elements.get('divider', [])
                line_geom = self.get_map_divider_geom(patch_box, patch_angle, divider_data, ego_SE3_city)
                ...
                ped_data = map_elements.get('ped_crossing', [])
                ped_geom = self.get_map_ped_geom(patch_box, patch_angle, ped_data, ego_SE3_city)
                ped_instance_list = self.ped_poly_geoms_to_instances(ped_geom)
                for instance in ped_instance_list:
                    vectors.append((instance, self.CLASS2LABEL.get('ped_crossing', -1)))
            elif vec_class == 'boundary':
                boundary_data = map_elements.get('boundary', [])
                polygon_geom = self.get_map_boundary_geom(patch_box, patch_angle, boundary_data, ego_SE3_city)
                ...
                centerline_data = map_elements.get('centerline', [])
                centerline_geom, kept_indices = self.get_map_centerline_geom(
                    patch_box, patch_angle, centerline_data, ego_SE3_city)
                line_instances_list = self.line_geoms_to_instances(centerline_geom)
                for centerline in line_instances_list:
                    vectors.append((centerline, self.CLASS2LABEL.get('centerline', -1)))
                if 'centerline_graph' in map_elements:
                    adj_matrix = self.get_centerline_adj_matrix(map_elements['centerline_graph'], kept_indices)
                else:
                    adj_matrix = np.zeros((len(line_instances_list), len(line_instances_list)), dtype=np.float32)

                # After geometry clipping/splitting, instance count can differ from graph nodes.
                # Align topology target size with current instances to avoid index out-of-bounds in training.
                if len(line_instances_list) == 0:
                    adj_matrix = np.zeros((0, 0), dtype=np.float32)
                elif adj_matrix.shape[0] != len(line_instances_list):
                    adj_matrix = np.zeros((len(line_instances_list), len(line_instances_list)), dtype=np.float32)
            else:
        ...
        gt_instance = LiDARInstanceLines(gt_instance,self.sample_dist,
                        ...
                        patch_size=self.patch_size, adj_matrix=adj_matrix, nums_control_pts=self.nums_control_pts)
    ...
    def get_map_centerline_geom(self, patch_box, patch_angle, centerlines, ego_SE3_city):
        map_centerline_geom = []
        ...
        map_centerline_geom.append(('centerline', line_list))
        return map_centerline_geom, kept_indices

    def get_centerline_adj_matrix(self, centerline_graph, kept_indices):
        if len(kept_indices) == 0:
            return np.zeros((0, 0), dtype=np.float32)

        kept_tokens = [centerline_graph[i][0] for i in kept_indices]
        token_to_new_idx = {token: i for i, token in enumerate(kept_tokens)}
        adj_matrix = np.zeros((len(kept_indices), len(kept_indices)), dtype=np.float32)

        for new_i, raw_i in enumerate(kept_indices):
            _, succ_ids = centerline_graph[raw_i]
            for succ in succ_ids:
                if succ in token_to_new_idx:
                    adj_matrix[new_i, token_to_new_idx[succ]] = 1
        return adj_matrix
...
class CustomAV2LocalMapDataset(CustomNuScenesDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        missing_centerline = 0
        for log_id, map_elements in self.id2map.items():
            if 'centerline' not in map_elements:
                missing_centerline += 1
                map_elements['centerline'] = map_elements.get('divider', [])
            if 'centerline_graph' not in map_elements:
                map_elements['centerline_graph'] = [
                    [idx, []] for idx in range(len(map_elements.get('centerline', [])))
                ]
        if missing_centerline > 0:
            print(f'[CustomAV2LocalMapDataset] patched missing centerline for {missing_centerline} logs in {ann_file}.')
        self.metadata = None
        self.version = None
        return data_infos
