...
class LiDARInstanceLines(object):
    ...
    def get_graph_gt_vis(self):
        ...
        for instance in self.instance_list:
            instance = instance.simplify(0.2, preserve_topology=True)  # 使用 simplify(0.2) 进行几何简化，容差0.2米 减少冗余点，使可视化更清晰
            sampled_points = np.array(instance.coords)
            line = to_tensor(sampled_points).to(dtype=torch.float32)
            line[:,0] = torch.clamp(line[:,0], min=-self.max_x,max=self.max_x)
            line[:,1] = torch.clamp(line[:,1], min=-self.max_y,max=self.max_y)
    ...
    def get_beizer_control_pts(self):
        ...
        """
        assert len(self.instance_list) != 0
        instance_points_list = []  # Step 1: 对每条车道线均匀采样 fixed_num 个点
        for instance in self.instance_list:
            distances = np.linspace(0, instance.length, self.fixed_num)
            sampled_points = np.array([list(instance.interpolate(distance).coords) for distance in distances]).reshape(-1, 2)
            instance_points_list.append(sampled_points)
        ...
        instance_points_tensor[:,:,0] = torch.clamp(instance_points_tensor[:,:,0], min=-self.max_x,max=self.max_x)
        instance_points_tensor[:,:,1] = torch.clamp(instance_points_tensor[:,:,1], min=-self.max_y,max=self.max_y)
        B = self.get_bernstein_basis(self.fixed_num * 2, self.nums_control_pts)  # Step 2: 计算 Bernstein 基矩阵的伪逆
        nums_line = len(self.adj_matrix)  # Step 3: 为每条车道对计算贝塞尔控制点
        ...
        for i in range(nums_line):
            connection = np.where(self.adj_matrix[i]==1)[0]
            for c in connection:
                new_line = torch.cat((instance_points_tensor[i], instance_points_tensor[c]))  # 拼接两条车道线的采样点
                control_pts = torch.matmul(inv_B, new_line)  # 通过伪逆矩阵求解控制点
...
class VectorizedLocalMap(object):
    ...
    def get_centerline_adj_matrix(self, centerlines, location, valid_records):
        ...
        for i in range(nums_centerline):
            ...
            outgoing_tokens = self.nusc_maps[location].get_outgoing_lane_ids(token)  # 从 NuScenes 地图 API 获取拓扑连接关系
            incoming_tokens = self.nusc_maps[location].get_incoming_lane_ids(token)
            for j in range(i+1, nums_centerline):
                if valid_records[j]['token'] in outgoing_tokens and (centerlines[i][-1]-centerlines[j][0]).sum()<0.01:  # 判断 i→j 的出边连接
                    adj_matrix[i][j] = 1
                if valid_records[j]['token'] in incoming_tokens and (centerlines[i][0]-centerlines[j][-1]).sum()<0.01:  # 判断 j→i 的入边连接
                    adj_matrix[j][i] = 1
    ...
    def get_center_line(self,patch_box,patch_angle,layer_name,location):
        ...
        patch_x = patch_box[0]  # 区域中心点x,y坐标
        patch_y = patch_box[1]

        patch = self.map_explorer[location].get_patch_coord(patch_box, patch_angle)  # 获取中心点60*30区域的信息
        ...
        for record in records:
            ...
            if polygon.is_valid:
                ...
                new_polygon = polygon.intersection(patch)
                if not new_polygon.is_empty:  # 当前车道的多边形与感兴趣区域有交集
                    centerline = self.map_explorer[location].map_api.discretize_lanes(
                            record, 0.5)  
                    centerline = list(self.map_explorer[location].map_api.discretize_lanes([record['token']], 0.5).values())[0]  # 使用 NuScenes API 的离散化方法,间隔 0.5m 采样
                    centerline = LineString(np.array(centerline)[:,:2])  # 转换为 LineString,只取 x,y 坐标(忽略 z)
                    centerline = centerline.intersection(patch)  # 裁剪到感兴趣区域
                    centerline = self.to_patch_coord(centerline, patch_angle, patch_x, patch_y)  # 坐标系转换到车辆局部坐标系
                    if not centerline.is_empty:
                        if centerline.geom_type == 'MultiLineString':  # 一条车道被patch切割成多段
...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        print_log('Results writes to ' + res_path, logger=None)
        ...
        print_log('Graph results writes to ' + res_graph_path, logger=None)
        ...
        print_log('Openlane results writes to ' + res_openlane_path, logger=None)
    ...
    def _evaluate_single(self,
        ...
        if "chamfer" in metric:
            ...
            for i, thr in enumerate(thresholds):
                print_log('-'*20 + f'threshold: {thr}' + '-'*20, logger=logger)
                mAP, cls_ap = eval_map(
                                gen_results,
                                annotations,
                                cls_gens,
            ...
            for i, name in enumerate(self.MAPCLASSES):
                print_log('{}: {}'.format(name, cls_aps.mean(0)[i]), logger=logger)
                detail['NuscMap/{}_AP'.format(name)] =  cls_aps.mean(0)[i]
            print_log('map: {}'.format(cls_aps.mean(0).mean()), logger=logger)
        ...
        if "topology" in metric:
            print_log('------Evaluate by topology------', logger=logger)
        ...
        if "openlane" in metric:
            # eval openlane topo  
            print_log('------Evaluate by openlane------', logger=logger)
